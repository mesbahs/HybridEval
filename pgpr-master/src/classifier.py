...
class classifier_method:
    ...
    def orderedlog_review_main(self):
        print('##### Ordered Logistic Regression #####')
        orderedlogreg = Pipeline([('vect', CountVectorizer()),
                           ('tfidf', TfidfTransformer()),
                           ('clf', LogisticAT(alpha=0)),
                           ])
        return orderedlogreg
